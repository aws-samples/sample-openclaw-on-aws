...
- **Non-root execution**: the image declares `USER agent`, so the container never runs as root — not at boot, not for the gateway, and not for any exec-tool command the agent runs. `OPENCLAW_HOME` is created and owned by `agent` at build time. An allowlist escape therefore cannot imply a root compromise of the instance. This is also a hard requirement on this compute type, not just hardening: AgentCore Runtime Instances starts the container on an id-mapped overlay mount where the container's uid 0 has no DAC override over `agent`-owned paths, so a root-then-`gosu` startup fails with EPERM before the agent can answer `/ping` (see [Serverless Considerations](./docs/SERVERLESS_CONSIDERATIONS.md#10-id-mapped-mounts-the-container-must-not-start-as-root)).
